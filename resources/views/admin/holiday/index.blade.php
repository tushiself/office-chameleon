...
@section('content')
        ...
        <div class="bg-white shadow-primary mt-5.5 rounded-10px">
            <div class="bg-lightgraybg p-5 rounded-t-10px border-b-2 border-gray text-right">
                <div class="flex items-center justify-between">
                    <h2 class="font-bold sm:text-base">All Holiday List</h2>
                    ...
                    <tbody>
                        @foreach ($holiday as $key => $holiday)
                            <tr>
                                <td>{{ ++$key }}</td>
                                <td>{{ $holiday->title }}</td>
                                <td style="text-align: left">{{ \Carbon\Carbon::parse($holiday->date)->format('d F Y') }}</td>
                                ...
                                @if (auth()->user()->role === 'Admin' || auth()->user()->role === 'Manager')
                                    <td>
                                        <div class="flex items-center gap-3">
                                            ...
                                            <button
                                                class="cursor-pointer text-lightgray duration-300 hover:text-red-400 delete-holiday-btn"
                                                data-id="{{ $holiday->id }}"><svg width="12" height="15"
    ...
    <div
        ...
        <div
            ...
            <h2 class="text-white bg-purple text-sm sm:text-base font-bold py-3.5 text-center">Add New Holiday</h2>
            <div class="bg-white p-3.5">
                <form id="addNewUserForm" action="{{ route('holiday.store') }}" method="POST" class="text-xs space-y-3.5">
                    @csrf
                    ...
                    <fieldset class="flex border-[1.5px] border-gray rounded-md overflow-hidden">
                        <label for="title" class="bg-purple inline-flex p-2.5"><svg width="14" height="14"
                        ...
                        <input id="title" placeholder="Holiday Title" type="text" name="title"
                            class="w-full outline-none px-2.5" required>
                    </fieldset>
                    <fieldset class="flex border-[1.5px] border-gray rounded-md overflow-hidden">
                        <label for="date" class="bg-purple inline-flex p-2.5"><svg width="14" height="14"
                                viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div
        ...
        <div
            class="z-10 w-full max-w-[320px] sm:max-w-[400px] bg-white absolute top-1/2 left-1/2 -translate-1/2 rounded-10px overflow-hidden">
            <h2 class="text-white bg-purple text-sm sm:text-base font-bold py-3.5 text-center">Edit Holiday</h2>
            <div class="bg-white p-3.5">
                <form id="addNewUserForm" method="POST" class="text-xs space-y-3.5">
                    @csrf
                    <input type="hidden" class="form-control holiday-id" name="holiday-id">
                    <fieldset class="flex border-[1.5px] border-gray rounded-md overflow-hidden">
                        <label for="title" class="bg-purple inline-flex p-2.5"><svg width="14" height="14"
                                viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
...
@section('custom-js')
    ...
    <script>
        $(document).on('click', '.edit-popup-button', function() {
            ...
            modal.find('input[name="date"]').val(date); // ✅ fixed selector
            ...
            form.attr('action', `/chameleon/holiday/${id}`);
            form.find('input[name="_method"]').remove(); // remove previous if any
    ...
    <script>
        $(document).ready(function() {
            $('.delete-holiday-btn').click(function() {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: 'You no longer want to keep this Holiday?',
                    icon: 'warning',
