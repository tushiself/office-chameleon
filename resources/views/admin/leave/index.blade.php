...
@section('content')
    ...
    <div class="mt-10 lg:mt-11">
        <div class="flex items-center justify-between">
            <h2 class="font-bold sm:text-base">My Leave List</h2>
            <button
                class="add-popup-button bg-purple text-white py-2.5 px-3 sm:px-5 rounded-md inline-flex gap-2 items-center sm:gap-4 font-semibold cursor-pointer text-xs sm:text-sm">
        ...
        <div class="bg-white shadow-primary mt-5.5 rounded-10px">
            ...
            <div class="p-5 lg:p-8 custom-table-main">
                <div class="flex flex-wrap items-center justify-center sm:justify-between text-xs md:text-sm gap-4">
                    <div class="flex flex-wrap gap-4 md:gap-12">
                        <div class="flex flex-col items-center font-medium gap-1">
                            <h3 class="font-bold text-4xl md:text-5xl text-purple">{{ $totalLeaves }}</h3>
                            <span>Paid Leave </span>
                        </div>
                        <div class="flex flex-col items-center font-medium gap-1">
                            <h3 class="font-bold text-4xl md:text-5xl text-green">{{ $remainingLeaves }}</h3>
                            <span>Paid Remaining</span>
                        </div>
                        <div class="flex flex-col items-center font-medium gap-1">
                            <h3 class="font-bold text-4xl md:text-5xl text-danger">{{ $usedLeaves }}</h3>
                            <span>Paid Used</span>
                        </div>
                        <div class="flex flex-col items-center font-medium gap-1">
                            <h3 class="font-bold text-4xl md:text-5xl text-primary">{{ $unpaidLeaves }}</h3>
    ...
    {{-- add leave --}}
    <div
        class="add-popup-main fixed z-40 lg:max-w-[calc(100%-386px)] xl:max-w-[calc(100%-406px)] w-full ml-auto inset-y-0 right-0 left-5 lg:left-10 xl:left-14 invisible opacity-0 duration-300">
        <div
            class="z-10 max-w-[680px] w-11/12 bg-white absolute top-1/2 left-1/2 -translate-1/2 rounded-10px overflow-hidden">
            ...
            <form action="{{ route('leave.store') }}" method="POST"
                class="p-5 md:py-8 md:px-7 font-medium rounded-b-10px space-y-3 md:space-y-6 text-xs overflow-scroll">
                @csrf

                @if (session('success'))
                    <div id="success-banner"
                        class="fixed top-0 left-1/2 transform -translate-x-1/2 bg-green-500 text-white py-3 px-6 rounded-b-lg shadow-lg z-50 transition-all duration-500 translate-y-[-100%]">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <fieldset>
                    ...
                    <div class="add-dropdown relative text-xs mt-3.5">
                        <span
                            class="h-11.5 flex items-center add-dropdown-select cursor-pointer w-full py-2 px-6 bg-white shadow-primary rounded-10px relative">
                            <span class="selected-text">Select leave type</span>
                            <svg class="add-dropdown-arrow duration-300 absolute right-6 top-5" width="11"
                                height="7" fill="none">
                        ...
                        <div
                            class="absolute inset-x-0 top-full w-full pt-2 z-10 add-dropdown-items invisible opacity-0 duration-300">
                            <div class="py-2.5 px-2 bg-white shadow-primary rounded-10px">
                                <input type="search"
                ...
                <fieldset>
                    ...
                    <input type="date" name="from_date" id="start_date"
                ...
                <fieldset>
                    ...
                    <input type="date" name="to_date" id="end_date"
                        class="w-full mt-3.5 bg-white shadow-primary py-3 px-6 rounded-10px outline-none">
                    @error('to_date')
                        <div class="text-red-600 mt-1">{{ $message }}</div>
                    @enderror
                ...
                <fieldset>
                    ...
                    <input type="number" name="total_days" id="days_count" value="1" readonly
                        class="w-full bg-white shadow-primary py-3 px-6 mt-3.5 rounded-10px outline-none">
                </fieldset>
                ...
                <fieldset class="flex gap-1.5 justify-center mt-9 text-sm">
                    ...
                    <button type="submit"
                        class="cursor-pointer bg-purple text-white font-semibold py-2.5 px-6 rounded-sm">Create</button>
                </fieldset>
            </form>
        </div>
        <div class="bg-black/10 absolute inset-0 add-popup-closebg"></div>
    </div>
@endsection
@section('custom-js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const popupMain = document.querySelector('.add-popup-main');
            const openCloseButtons = document.querySelectorAll(
                '.add-popup-button, .add-popup-close, .add-popup-closebg');
            ...
            const trigger = dropdown.querySelector(".add-dropdown-select");
            const dropdownItems = dropdown.querySelector(".add-dropdown-items");
